<?php
	...
	if(isset($_POST["send"])){
		...
		if(isset($_POST["captcha"])){
		    ...
		    $result_query_select = $mysqli->query("SELECT email_status FROM `users` WHERE user_email = '".$email."'");
		        ...
		        //Проверяем, если в базе нет пользователя с такими данными, то выводим сообщение об ошибке
		        	...
		        	while(($row = $result_query_select->fetch_assoc()) !=false){
		        	    ...
		        	    if((int)$row["email_status"] === 0){
		        	    	...
		        	    	$query_update_token = $mysqli->query("UPDATE users SET reset_password_token='$token' WHERE user_email='$email'");
